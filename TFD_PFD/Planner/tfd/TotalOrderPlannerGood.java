...
//import fr.uga.pddl4j.tutorial.tfd.Node1TFD;

public class TotalOrderPlanNode1 {

	  private Properties arguments;

	  public TotalOrderPlanNode1(final Properties arguments) {
	    super();
...
//		  final PriorityQueue<Node1> openList = new PriorityQueue<>(100, new Comparator<Node1>() {
//			    public int compare(Node n1, Node n2) {
//			       int taskList1 = n1.getTasksList().size();
//			       int taskList2 = n2.getTasksList().size();
//			      return Integer.compare(taskList1, taskList2);
//			    }
//			  });

		 final Node1 root = new Node1(problem.getInitialState(), problem.getInitialTaskNetwork().getTasks());
		  ...
		  while (!openList.isEmpty() && plan==null ) {
			...
			final Node1 Node1InProgress = openList.poll();
			
			if (Node1InProgress.getTasksList().isEmpty()) {
				plan = this.extractPlan(Node1InProgress, problem);
			} else {
				//tasklist is not empty for our current Node1 being processed
				// we pop the first task in the tasklist of our curent Node1
				int taskPop = Node1InProgress.getTasksList().pop();
				final State stateTaskPop = Node1InProgress.getState();
				...
				if (problem.getTasks().get(taskPop).isPrimtive()) {
					// taskpop is primitive now we try to add a new Node1 in openlist who is produced by this task
					for (Integer op : active) {
						...
						//verified that state of our Node1 correspond to the state of application action can resolve our task
						if (stateTaskPop.satisfy(action.getPreconditions())){
							final Node1 successorNode1 = new Node1(Node1InProgress);
							//successor contains the next task of current Node1(tasklist) and the same state
							// we are going to change all know...
							successorNode1.getState().apply(action.getCondEffects());
							successorNode1.setParent(Node1InProgress);
							successorNode1.setOperator(op);
							
							//
							//System.out.println("premiere action : "+action.getName());
							//add new Node1 in top of open list
							openList.add(successorNode1);
						}
					}
					...
					for (Integer op : active) {
						...
						if (stateTaskPop.satisfy(method.getPreconditions())){
							...
							successorNode1.setOperator(op + problem.getActions().size());
							successorNode1.setParent(Node1InProgress);
							//
							System.out.println("list avant ajout tache methode : "+successorNode1.getTasksList());
							System.out.println("list de tache parend Node1 : "+Node1InProgress.getTasksList());
							
							//System.out.println("sous tache de la methode : "+method.getName()+" : est : "+method.getSubTasks()+"\n");
							//List<Integer> taskList = successorNode1.getTasksList();
	
							//taskList.addAll(0, method.getSubTasks()); //taskList.addAll(0, subTasks);
				   
							//successorNode1.setTasksList(tasksList);
							
							successorNode1.putListSubTasksAtTop(method.getSubTasks());
							//
							System.out.println("list apres ajout tache methode : "+successorNode1.getTasksList());
							System.out.println("premiere methode : "+method.getName());		
							System.out.println("noeud construit n°: "+i+" est "+successorNode1.getTasksList()+"\n\n");
	  ...
	  /**
	     * Extract a plan from a solution Node1 for the specified planning problem.
	     *
	     * @param Node1    the solution Node1.
	     * @param problem the problem to be solved.
	     * @return the solution plan or null is no solution was found.
	     */
	    private SequentialPlan extractPlan(final Node1 Node1, final Problem problem) {
	        Node1 n = Node1;
	    ...
	    public static void main(final String[] args) {
	        ...
	        final Properties arguments = TotalOrderPlanNode1.parseCommandLine(args);
	        if (arguments == null) {
	            TotalOrderPlanNode1.printUsage();
	            System.exit(0);
	        }

	        // Create an instance of the TFDPlanner Planner
	        final TotalOrderPlanNode1 planner = new TotalOrderPlanNode1(arguments);
